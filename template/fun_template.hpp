 ...
 * 2.模板类型参数也可以是非类型的参数，但是非类型模板参数不代表类型，而代表一个常量，且必须是一个常量。
 *   以保证在编译阶段知道值，可以实例化出一个类型。两种大的类型可以作为非类型模板参数的形参:
 *  1.整型，注意char、short、long、long long都是整型.但是浮点型不可以
 *  2.对象类型的指针或者左值引用，这里的对象可以是内置类型、也可以是类类型
 *  绑定到非类型模板参数的整型实参必须是一个常量表达式，绑定到非类型模板参数的指针或者引用的实参必须具有静态生存期。总结来说
 *  就是传递给模板非类型模板参数的实参必须是一个常量表达式。查阅网上资料有说到"非类型模板参数可以是指针，但该指针必须指向外部链接对象",
 ...
 *      的...表示的是对模板参数包的扩展
 * 2.在模板参数的位置也可以表示非类型模板参数包,表示非类型模板参数包的方法是在一个明确的类型后面跟一个省略号,例如:template<int...Args>,此处的Args表示零个或者多个int类型的非类型模板参数
...
//模板函数偏特化,c++是不支持模板函数偏特化的，但通常有对模板函数偏特化的需求，就是部分模板参数在特定类型下使用特定函数。
//这个时候可以借助模板函数重载来实现模板函数偏特化的效果。基于模板函数重载的特性还可以衍生处标签分发的方式实现模板函数偏特化
//以下是模板函数重载演示:
