 ...
 *3.可以将基类的指针或引用绑定到派生类对象上。当使用基类的引用或者指针时，实际上我们并不清楚该引用(指针)所绑
 ...
 *  的一部分，则它只含有基类定义的成员，而不含有派生类定义的成员。
 *  因为基类的对象可能是派生类的一部分，也可能不是，所以不存在从基类向派生类自动类型转换。即使一个基类的指针或引用绑定在一个派生类对象上，
 *  我们也不能执行从基类向派生类型的转换。编译器在编译期无法确定某个特定的转换在运行时是否安全，这是因为编译器只能通过检查指针
 ...
 * 它的派生类部分将被忽略掉。尽管了自动类型转换只对指针或引用类型有效，但是继承体系中的大多数类仍然定义了拷贝控制成员。
 * 因此通常能够将一个派生类对象拷贝、移动或者赋值给一个基类对象。在拷贝、移动、赋值过程中调用的是基类的对应函数。
 * 不过这种操作只能处理派生类对象的基类部分。
 ...
 * 15.派生类的成员或者友元只能通过访问派生类对象中的基类部分的受保护成员，对于普通的基类对象中的成员不具有特殊的访问权限。
 ...
 * 19.当继承关系存在时派生类的作用域嵌套在其基类作用域中。名字查找是在编译时进行的，所以一个对象、引用或指针的静态
 * 类型决定了该对象的哪些成员是可见的。即使静态类型和动态类型不一致的情况下，我们能使用哪些成员仍然是由静态类型决定的。
 * 由于派生类嵌套在基类作用域中，所以名字冲突时内层隐藏外层，即派生类隐藏基类。假如派生类定义了和基类同名的成员变量，通过
 * 派生类访问时访问的是派生类自己的。
...
class Bulk_quote:public Quote
{
    public:
        ...
        double net_price (std::size_t) const override;//c++11特性，在函数参数列表后加override显式的说明该函数是覆盖了其继承的基类中的函数
...
//抽象基类
//在Bulk_quote1和Quote之间加了一层Disc_Quote，这样重新设计了类的体系以便于将
//操作或者数据从一个类移动到另外一个类中。虽然我们修改整个继承体系，但是使用了Bulk_quote1的代码
//无需进行任何改动。例如:Bulk_quote1之前直接继承Quote，外部代码直接初始化Bulk_quote1,然后使用Bulk_quote1，
//在在Bulk_quote1和Quote之间加了一层Disc_Quote后，外部代码不会做任何改动，依旧可以使用,但是整个继承体系却发生了变化。
//这种行为就是重构
class Disc_quote:public Quote
{
    ...
    protected:
        std::size_t quantity = 0; //折扣适用的购买数量
        double discount = 0.0; //折扣
};
...
class A2:public BaseA, public BaseB
{
